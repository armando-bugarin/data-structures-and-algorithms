...
const count = (target, input) => {
  ...
  return input.reduce((total, arr) => total + arr.filter(num => num === target).length, 0);
...
const totalSum = (input) => {
  ...
  return input.reduce((total, arr) => total + arr.reduce((total, num) => total + num, 0), 0);
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map(arr => arr.filter(num =>!isNaN(num) && num % 5 === 0).map(num => Math.pow(2, num)));
...
let findMaleAndFemale = (data) => {
  ...
  return data.filter(obj => obj.gender ==='male' || obj.gender === 'female').map(obj => obj.name).join(' and ');
...
let findShortest = (data) => {
  ...
  return data.filter(obj => obj.height).sort((a, b) => a.height - b.height)[0].name;
