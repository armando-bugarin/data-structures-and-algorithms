...
const typeNum = (arr) => {
  const output = arr.filter(item => typeof item === 'number');
...
const containsAnd = (arr) => {
  const output = arr.filter(item => item.includes('and'));
...
const notInFirstArray = (forbiddenValues, arr) => {
  const output = arr.filter(item =>!forbiddenValues.includes(item));
